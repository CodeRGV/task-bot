...
module.exports = function(controller){
	...
	Channel.prototype.init = (function(perform){
		...
		name: function(name){
			var self = this;
			if (!name){
				var i = this.pending.push(this.getName().then(function(name){
					self.setName(name);
					self.save();
				}).fail(function(err){
	...
	getName.im = function(deferred){
		...
		api.im.history({channel: this.json.id}, function(err, res){
			...
			var i = self.pending.push(User.find(message.user).then(function(user){
				var j = self.pending.push(user.getName().then(function(name){
	...
	Channel.prototype.getPending = function(){
		return this.pending;
	};
